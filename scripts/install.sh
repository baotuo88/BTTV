...
# 是否开放注册
ALLOW_REGISTER=$(read_input "是否开放用户注册 (true/false)" "true")
...
# 是否开放新用户注册（可在后台“运营配置”中调整）
ALLOW_REGISTER=${ALLOW_REGISTER}
# 存储模式：local 使用服务器本地 Docker 数据卷；mongodb 使用外部 MongoDB
