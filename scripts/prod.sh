...
if [[ $PUSH_TO_HUB =~ ^[Yy]$ ]]; then
  ...
  IMAGE_NAME="${DOCKER_USERNAME}/bttv"
...
APP_PORT=$(grep -A 2 "container_name: bttv-app" docker-compose.yml | grep "ports" -A 1 | grep -o '[0-9]*:3000' | cut -d':' -f1)
