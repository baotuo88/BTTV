    ...
    const fetchVodSources = async () => {
      try {
        ...
        if (response.status === 401) {
          ...
          setError(result?.message || "请先登录或注册后播放");
          setLoading(false);
          return;
        }
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        if (result.code !== 200) {
          throw new Error(result.msg || result.message || "获取影视详情失败");
        }
        ...
        const message = err instanceof Error ? err.message : "获取影视详情失败，请稍后重试";
        setError(message);
      } finally {
        ...
        <div className="text-center px-6">
          ...
          <div className="mt-4 flex items-center justify-center gap-3">
            {needAuth && (
              <>
                <button
                  onClick={() => router.push(`/user/login?redirect=${encodeURIComponent(`/play/${dramaId}${currentSourceKey ? `?source=${currentSourceKey}` : ""}`)}`)}
