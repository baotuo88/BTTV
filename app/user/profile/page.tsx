...
export default function UserProfilePage() {
  ...
  const hasLibraryData = useMemo(
    () => Object.values(library).some((items) => items.length > 0),
    [library]
  );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[#8c8c8c] mt-1">管理账号、清单与云端观看进度</p>
          </div>
          <Link
            href="/"
        ...
        >
          ...
          {!hasLibraryData && (
            <p className="text-sm text-[#8c8c8c]">你还没有添加任何收藏/清单。</p>
          )}
                ...
                <div className="space-y-2 max-h-72 overflow-auto">
                  {library[listType].length === 0 ? (
                    ...
                    library[listType].map((item) => (
                      ...
                      >
                        <Link
                          href={item.sourceKey ? `/play/${item.itemId}?source=${item.sourceKey}` : `/play/${item.itemId}`}
