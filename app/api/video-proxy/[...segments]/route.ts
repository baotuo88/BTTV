...
export async function GET(
  ...
  if (playbackAuthError) {
    return playbackAuthError;
  }
  ...
  try {
    ...
    // 共享的总预算：Vercel serverless 硬上限是 10 秒，这里留 2 秒给上下游处理，
    // 所有策略总共只能用 8 秒。每个策略取剩余时间与 5 秒的较小值作为单次超时。
    const TOTAL_BUDGET_MS = 8000;
    const PER_STRATEGY_CAP_MS = 5000;
    ...
    // 如果所有策略都失败或没能成功响应
    if (!response) {
      throw lastError || new Error('所有请求策略失败或超出预算');
    }
