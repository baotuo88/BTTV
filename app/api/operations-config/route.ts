...
function sanitizePayload(input: unknown): Partial<OperationsConfigData> {
  ...
  if (source.allowRegister !== undefined) {
    payload.allowRegister = Boolean(source.allowRegister);
  }
  if (source.allowGuestPlay !== undefined) {
    payload.allowGuestPlay = Boolean(source.allowGuestPlay);
  }
