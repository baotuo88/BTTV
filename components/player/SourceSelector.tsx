...
interface SourceSelectorProps {
  ...
  dramaLockedSourceKey?: string;
  globalLockedSourceKey?: string;
  onToggleDramaLock?: (sourceKey: string) => void;
  onToggleGlobalLock?: (sourceKey: string) => void;
}
...
export function SourceSelector({
  ...
  currentSourceKey,
  onSourceChange,
  dramaLockedSourceKey = "",
  globalLockedSourceKey = "",
  onToggleDramaLock,
  onToggleGlobalLock,
}: SourceSelectorProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {!dramaLockedSourceKey && globalLockedSourceKey && (
        ...
        <div className="absolute right-0 mt-3 w-[min(82vw,17rem)] md:w-80 max-h-[calc(100dvh-96px)] md:max-h-none pb-safe-area-inset-bottom bg-gray-900/98 backdrop-blur-xl rounded-xl shadow-2xl border border-gray-700 overflow-hidden animate-fade-in z-50 flex flex-col">
          ...
          <div className="flex-1 min-h-0 md:max-h-[60vh] overflow-y-auto">
            {sortedSources.map((source, index) => {
              ...
              const isDramaLocked = dramaLockedSourceKey === source.source_key;
              const isGlobalLocked = globalLockedSourceKey === source.source_key;
              return (
                <div
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onToggleDramaLock?.(source.source_key);
                          }}
                        ...
                        >
                          {isDramaLocked ? "取消本剧锁" : "设为本剧锁"}
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onToggleGlobalLock?.(source.source_key);
                          }}
                          className={`text-[10px] px-2 py-0.5 rounded border ${
                            isGlobalLocked
