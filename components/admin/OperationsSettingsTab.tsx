...
function normalizeConfig(config: OperationsConfigData): OperationsConfigData {
  return {
    ...
    allowGuestPlay: config.allowGuestPlay ?? true,
  ...
  return (
    <div className="space-y-6">
      <div className="bg-[#1a1a1a] rounded-xl p-6 border border-[#333] space-y-6">
        ...
        <section className="space-y-3 border border-[#333] rounded-lg p-4 bg-[#141414]">
          <h3 className="text-white font-semibold">账号注册</h3>
          <label className="inline-flex items-center gap-2 text-sm text-[#b3b3b3]">
            <input
              type="checkbox"
              checked={formData.allowRegister}
              onChange={(event) =>
                setFormData((prev) => ({
                  ...prev,
                  allowRegister: event.target.checked,
                }))
              }
            />
            开放用户注册
          ...
          <p className="text-xs text-[#8c8c8c]">
            关闭后，前台将隐藏“注册”入口，且接口会拒绝新用户注册请求。
          </p>
        </section>

        <section className="space-y-3 border border-[#333] rounded-lg p-4 bg-[#141414]">
          <h3 className="text-white font-semibold">游客播放权限</h3>
          <label className="inline-flex items-center gap-2 text-sm text-[#b3b3b3]">
            <input
              type="checkbox"
              checked={formData.allowGuestPlay}
              onChange={(event) =>
                setFormData((prev) => ({
                  ...prev,
                  allowGuestPlay: event.target.checked,
                }))
              }
            />
            允许未登录用户播放
          ...
          <p className="text-xs text-[#8c8c8c]">
            关闭后，影视播放相关接口会要求先登录，未登录用户将无法加载播放数据。
          </p>
        </section>
