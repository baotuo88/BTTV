...
// 豆瓣数据服务地址。必须通过 NEXT_PUBLIC_DOUBAN_API_URL 环境变量显式配置。
// 未配置时以空字符串占位——所有 fetch 会失败并在控制台报错，避免静默走到不受你控制的第三方后端。
const DOUBAN_API_URL = process.env.NEXT_PUBLIC_DOUBAN_API_URL || ''
