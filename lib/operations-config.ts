...
import { getDefaultAllowRegister, resolveAllowRegister } from "@/lib/register-policy";
import type {
...
const DEFAULT_ALLOW_REGISTER = getDefaultAllowRegister();
...
function mergeWithDefault(config: Partial<OperationsConfigData>): OperationsConfigData {
  return {
    ...
    allowRegister: resolveAllowRegister(
      config.allowRegister,
      DEFAULT_OPERATIONS_CONFIG.allowRegister
    ),
    allowGuestPlay: resolveAllowRegister(
      config.allowGuestPlay,
      DEFAULT_OPERATIONS_CONFIG.allowGuestPlay
    ),
  ...
  await collection.updateOne(
    { id: 1 },
    {
      $set: {
        ...
        allowRegister: merged.allowRegister,
        allowGuestPlay: merged.allowGuestPlay,
        updatedAt: now,
      },
    },
    { upsert: true }
